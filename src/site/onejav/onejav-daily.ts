...
export async function uploadDaily(pathDate: string, sisterNumber: number, loadCompleted: boolean, retry = 3) {
	...
	console.log('上传每日数据');
	onejav.updateOne({pathDate}, daily, {upsert: true}).then(() => {
...
export async function loadDailies() {
	...
	getOnejavDaily().find({}).then((result) => {
		for (let onejavDaily of result) {
...
function updateOrAddDaily(daily: OnejavDaily) {
	...
	let index = dailies_value.findIndex((item: OnejavDaily) => item.pathDate === daily.pathDate);
	if (index >= 0) {
		// 如果有，就更新该对象
