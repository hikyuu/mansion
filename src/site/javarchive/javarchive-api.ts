...
export async function getDetailFromJavStore(javarchive: string, retry = 3): Promise<Document | undefined> {
  try {
    const result = await request(javarchive, 'https://javarchive.com/')
    return parseText(result.responseText)
  } catch (reason) {
    console.error(reason)
    if (retry > 0) {
      console.log('重试获取图片')
      return getDetailFromJavStore(javarchive, --retry)
    }
    return undefined
  }
}
